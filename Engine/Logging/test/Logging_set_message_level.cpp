...
// $Id: //depot/main/Engine/Logging/test/Logging_set_message_level.cpp#3 $
// $DateTime: 2009/07/30 12:22:26 $

// Unit-test for Engine.Logging set_message_level().
...
/// Checks how set_message_level() works.
void test_message_level()
{
   ...
   Logger::set_message_level(0, critical);
   Logger::set_message_level(1, major);
   Logger::set_message_level(2, minor);
   Logger::set_message_level(3, trivial);

   // check that for all senders except 0-3 message level remains "trivial"
   BOOST_CHECK(Logger::get_message_level(0) == critical);
   BOOST_CHECK(Logger::get_message_level(1) == major);
   BOOST_CHECK(Logger::get_message_level(2) == minor);
   BOOST_CHECK(Logger::get_message_level(3) == trivial);
   for (uchar sender_id = 4; sender_id < numeric_limits<uchar>::max(); ++sender_id)
   {
      BOOST_CHECK(Logger::get_message_level(sender_id) == trivial);
   }

   // write some test messages to log
   // note, that only messages with level greater than that set up to their sender should be logged
...
{
   boost::unit_test::test_suite* test = BOOST_TEST_SUITE("Logging_set_message_level");
